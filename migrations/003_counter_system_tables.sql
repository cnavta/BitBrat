-- Migration: 003_counter_system_tables.sql (CORRECTED)
-- Sprint: sprint-27-6tp11t
-- Description: Create counter_definitions and counter_snapshots tables for counter system
-- Date: 2026-08-29
-- Schema: JSONB-based DocumentStore pattern (NOT relational columns)
--
-- IMPORTANT: PostgresDocumentStore uses JSONB storage with a single 'data' column,
-- not individual columns for each field. This migration was initially created
-- incorrectly with relational schema, causing error 42703 (column does not exist).
...
BEGIN
  ...
  RAISE NOTICE 'Migration 003_counter_system_tables.sql completed successfully - JSONB schema';
END $$;
