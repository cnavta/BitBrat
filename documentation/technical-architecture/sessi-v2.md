...
#### 2. Core Concepts
- **Stream**: A filtered, time-bound collection of events retrieved from the `PersistenceStore` (PostgreSQL or Firestore legacy).
...
- **Source**: Database (`events`, `prompt_logs` tables/collections).
- **Trigger**: 
...
2. **Extraction Phase**: Queries the database for events within the specified `window` and matching `filters`.
...
- **Phase 1**: `stream-analyst-service` core (database source via PersistenceStore, LLM integration).
