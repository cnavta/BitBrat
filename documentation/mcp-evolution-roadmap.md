...
### 3.1 Database Tool Registry
- **Schema Definition**: Formalize a database schema for `BitBratTool` persistence, including JSON Schema, metadata, and category tags.
- **DatabaseToolProvider**: Implement a provider that fetches tool definitions from database collections/tables.
- **Embedding Layer**: Integrate a mechanism to generate vector embeddings for tool descriptions.

### 3.2 Semantic Tool Selection
- **Contextual Search**: For every incoming `llm-bot` event, embed the user prompt and perform a similarity search (using database vector search) for relevant tools.
...
- `manage_personality`: Create, update, or archive personality documents in the database.
...
## 6. Proposed Next Sprint: "The Database Bridge" (Sprint 161)
1. **Tool Persistence**: Create database collections/tables for `tools` and `mcp_servers`.
2. **Dynamic Provider**: Implement `DatabaseToolProvider` and integrate it into `ToolRegistry`.
3. **Admin Tools**: Implement `register_tool` and `update_personality` as MCP tools.
4. **Refactor**: Update `llm-bot` startup to initialize both local and database-based tools.
...
- **Observability**: Track tool selection accuracy and execution success rates in structured logging.
