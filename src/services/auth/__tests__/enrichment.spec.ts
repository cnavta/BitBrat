...
describe('enrichEvent()', () => {
  ...
  test('matched by id populates user and auth.matched=true', async () => {
    ...
    expect((res.event as any).user).toEqual({ id: 'u-1', email: 'a@b.c', displayName: 'Alice' });
    expect((res.event as any).auth).toEqual({ v: '1', method: 'enrichment', matched: true, at: fixedNow(), provider: 'twitch', userRef: 'users/u-1' });
  });
