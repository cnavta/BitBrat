 ...
 */
export function normalizeAttributes(input: Record<string, unknown> | undefined | null): Record<string, string> {
...
function toLowerCamelCase(s: string): string {
  ...
  const parts = s.replace(/[^a-zA-Z0-9]+/g, ' ').trim().split(' ').map(p => p.toLowerCase());
