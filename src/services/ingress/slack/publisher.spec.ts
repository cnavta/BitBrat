import { SlackIngressPublisher } from './publisher';
...
describe('SlackIngressPublisher', () => {
  beforeEach(() => {
  ...
  it('publishes to ${BUS_PREFIX}internal.ingress.v1 with attributes (V2)', async () => {
    ...
    const pub = new SlackIngressPublisher({ busPrefix: 'dev.' });
    ...
    expect(call.attrs.source).toBe('ingress.slack');
  ...
  describe('onPublished callback (Sprint 24)', () => {
    it('calls onPublished callback after successful publish', async () => {
      ...
      const pub = new SlackIngressPublisher({
        busPrefix: '',
      ...
      const evt: InternalEventV2 = {
        v: '2',
        ingress: { ingressAt: new Date().toISOString(), source: 'ingress.slack' },
        identity: { external: { id: 'u1', platform: 'slack' } },
        correlationId: 'c-snapshot-1',
        type: 'chat.message.v1',
        message: { id: 'm1', role: 'user', text: 'test' },
        egress: { destination: '', connector: 'slack' as any }
      } as any;
    ...
    it('does not call onPublished if callback not provided', async () => {
      const pub = new SlackIngressPublisher({ busPrefix: '' });
