...
class SchedulerServer extends BaseServer {
  constructor() {
  ...
  private async setupApp(app: Express, _cfg: any) {
    ...
    try {
      ...
      { // subscription for internal.scheduler.tick
        const raw = "internal.scheduler.tick";
        ...
        try {
          await this.onMessage<InternalEventV2>(
            ...
            async (msg: InternalEventV2, _attributes, ctx) => {
              try {
                this.getLogger().info('scheduler.message.received', {
                  destination,
                ...
                this.getLogger().error('scheduler.message.handler_error', { destination, error: e?.message || String(e) });
                await ctx.ack();
              }
            }
          );
          this.getLogger().info('scheduler.subscribe.ok', { destination, queue });
        } catch (e: any) {
          this.getLogger().error('scheduler.subscribe.error', { destination, queue, error: e?.message || String(e) });
        }
      }
    } catch (e: any) {
      this.getLogger().warn('scheduler.subscribe.init_error', { error: e?.message || String(e) });
    }
