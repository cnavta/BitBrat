...
class ApiGatewayServer extends BaseServer {
  constructor() {
  ...
  private async setupApp(app: Express, _cfg: any) {
    ...
    try {
      ...
      { // subscription for internal.api.egress.v1.{instanceId}
        const raw = "internal.api.egress.v1.{instanceId}";
        ...
        try {
          await this.onMessage<InternalEventV2>(
            ...
            async (msg: InternalEventV2, _attributes, ctx) => {
              try {
                this.getLogger().info('api-gateway.message.received', {
                  destination,
                ...
                this.getLogger().error('api-gateway.message.handler_error', { destination, error: e?.message || String(e) });
                await ctx.ack();
              }
            }
          );
          this.getLogger().info('api-gateway.subscribe.ok', { destination, queue });
        } catch (e: any) {
          this.getLogger().error('api-gateway.subscribe.error', { destination, queue, error: e?.message || String(e) });
        }
      }
    } catch (e: any) {
      this.getLogger().warn('api-gateway.subscribe.init_error', { error: e?.message || String(e) });
    }
