        ...
        try {
          await this.onMessage<InternalEventV2>(
            ...
            async (msg: InternalEventV2, _attributes, ctx) => {
              try {
                this.getLogger().info('auth.message.received', {
                  destination,
                ...
                this.getLogger().error('auth.message.handler_error', { destination, error: e?.message || String(e) });
                await ctx.ack();
              }
            }
          );
          this.getLogger().info('auth.subscribe.ok', { destination, queue });
        } catch (e: any) {
          this.getLogger().error('auth.subscribe.error', { destination, queue, error: e?.message || String(e) });
        }
      }
    } catch (e: any) {
      this.getLogger().warn('auth.subscribe.init_error', { error: e?.message || String(e) });
    }
