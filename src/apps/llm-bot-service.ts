...
class LlmBotServer extends BaseServer {
  ...
  private async setupApp(app: Express, _cfg: any) {
    ...
    app.get('/_debug/mcp', (_req: Request, res: Response) => { res.status(200).end(); });
    ...
    try {
      ...
      { // subscription for internal.llmbot.v1
        const raw = "internal.llmbot.v1";
        ...
        try {
          await this.onMessage<InternalEventV2>(
            ...
            async (msg: InternalEventV2, _attributes, ctx) => {
              try {
                this.getLogger().info('llm-bot.message.received', {
                  destination,
                ...
                this.getLogger().error('llm-bot.message.handler_error', { destination, error: e?.message || String(e) });
                await ctx.ack();
              }
            }
          );
          this.getLogger().info('llm-bot.subscribe.ok', { destination, queue });
        } catch (e: any) {
          this.getLogger().error('llm-bot.subscribe.error', { destination, queue, error: e?.message || String(e) });
        }
      }
    } catch (e: any) {
      this.getLogger().warn('llm-bot.subscribe.init_error', { error: e?.message || String(e) });
    }
