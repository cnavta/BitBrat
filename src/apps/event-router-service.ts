...
class EventRouterServer extends BaseServer {
  ...
  private async setupApp(app: Express, _cfg: any) {
    ...
    try {
      ...
      { // subscription for internal.user.enriched.v1
        const raw = "internal.user.enriched.v1";
        ...
        try {
          await this.onMessage<InternalEventV2>(
            ...
            async (msg: InternalEventV2, _attributes, ctx) => {
              try {
                this.getLogger().info('event-router.message.received', {
                  destination,
                ...
                this.getLogger().error('event-router.message.handler_error', { destination, error: e?.message || String(e) });
                await ctx.ack();
              }
            }
          );
          this.getLogger().info('event-router.subscribe.ok', { destination, queue });
        } catch (e: any) {
          this.getLogger().error('event-router.subscribe.error', { destination, queue, error: e?.message || String(e) });
        }
      }
    } catch (e: any) {
      this.getLogger().warn('event-router.subscribe.init_error', { error: e?.message || String(e) });
    }
