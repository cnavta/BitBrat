...
describe('extract-config CLI', () => {
  it('emits oauth-flow config merged from architecture.yaml', () => {
    ...
    expect(json.ENV_KEYS).toEqual(['LOG_LEVEL','MESSAGE_BUS_DRIVER','NATS_URL','BUS_PREFIX']);
    expect(json.SECRETS).toEqual(['TWITCH_CLIENT_ID','TWITCH_CLIENT_SECRET','OAUTH_STATE_SECRET']);
    expect(json.SECRET_SET_ARG).toBe('TWITCH_CLIENT_ID=TWITCH_CLIENT_ID:latest;TWITCH_CLIENT_SECRET=TWITCH_CLIENT_SECRET:latest;OAUTH_STATE_SECRET=OAUTH_STATE_SECRET:latest');
  });
});
