...
- [x] BaseServer.CONFIG_DEFAULTS introduced and honored by getConfig(name) as a fallback when no env/default is present (secrets do not use class defaults).
...
## Validation Summary
- npm install/build: success (post-change build also successful)
