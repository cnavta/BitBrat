...
- Publication phase — created GitHub Pull Request
  - Command:
    - gh pr create --title "Sprint sprint-108-bc7a2d Deliverables – Command Processor (first pass)" \
      --body "Generated by LLM agent according to Sprint Protocol v2.4." \
      --base main --head feature/sprint-108-bc7a2d-command-processor-architecture
  - PR URL: https://github.com/cnavta/BitBrat/pull/12
  - Updated planning/sprint-108-bc7a2d/publication.yaml with status=created

## 2025-12-01T20:45:00Z
- Fix tests: error handling policy nack(true) on publish failure
  - Root cause: processEvent was imported statically in service; jest.doMock override in tests didn't apply → no publish path → nack not called
  - Changes:
    - src/apps/command-processor-service.ts: lazy-require processor inside handler to honor jest.doMock
    - src/apps/command-processor-service.ts: subscription disable logic now only respects MESSAGE_BUS_DISABLE_SUBSCRIBE (do not auto-disable under Jest)
    - src/apps/command-processor-service.ts: add RoutingStep typing for slip findIndex (fix TS implicit any)
  - Results:
    - npm test → All suites passing (1 skipped); failing spec now green
  - Git:
    - git add -A
