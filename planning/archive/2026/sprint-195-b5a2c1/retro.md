...
- Identified and fixed NATS healthcheck failure caused by missing `curl`/`wget` in the default `nats:2` scratch image. Switched to `nats:2-alpine` and used `wget` which is included in BusyBox.
