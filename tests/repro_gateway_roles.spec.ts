...
describe('Tool Gateway RBAC Repro', () => {
  ...
  it('GET /v1/tools without roles returns only public tools', async () => {
    ...
    expect(res.body.tools.length).toBe(9); // 2 public + 1 internal (agent.sendProgressUpdate) + 6 composition tools (Sprint 41 fix)
  ...
  it('GET /v1/tools with admin role returns matching tools', async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body.tools.length).toBe(14); // 2 public + 5 admin + 1 internal + 6 composition tools (Sprint 41 fix) (admin:llm excluded - no agent match)
  });
  ...
  it('GET /v1/tools for llm-bot agent returns allowlisted tools even without roles', async () => {
    ...
    expect(res.status).toBe(200);
    // Should see 2 public + 1 admin:llm + 1 internal + 6 composition tools (Sprint 41 fix) = 10 tools
    expect(res.body.tools.length).toBe(10);
