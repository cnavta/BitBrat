/**
 * `brat release` CLI group (sprint-326, BL-326-300) — a thin shell over the release module
 * (tools/brat/src/release). Orchestrates: compute next SemVer -> write architecture.yaml (only
 * project.version) + package.json -> sync package-lock.json -> assert 3-file consistency -> roll the
 * CHANGELOG `[Unreleased]` block -> (optional) local git tag.
 *
...
export async function cmdRelease(cmd: string[], rest: string[], flags: ReleaseCliFlags, logger?: Logger): Promise<void> {
  ...
  const bump = cmd[1];
  if (!bump) {
    console.error('Usage: brat release <patch|minor|major|x.y.z> [--dry-run] [--tag] [--yes]');
