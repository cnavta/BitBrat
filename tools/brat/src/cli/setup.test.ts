...
describe('Setup Utilities', () => {
  ...
  describe('getInitialRoutingRules', () => {
    it('should return rules with correct RuleDoc structure', () => {
      ...
      const botRule = rules.find(r => r.id === 'contextualization-reaction-bot');
      expect(botRule?.logic).toContain(botName);

      // Rule 1: initial-contextualization
      const rule1 = rules.find(r => r.id === 'initial-contextualization');
      expect(rule1?.routing.stage).toBe('contextualization');
      expect(rule1?.routing.slip.some((s: any) => s.attributes)).toBe(false);

      // Rule 2: contextualization-reaction-bot
      expect(botRule?.routing.stage).toBe('reaction');
      expect(botRule?.routing.slip.some((s: any) => s.attributes)).toBe(false);

      // Rule 3: contextualization-reaction-adventure
      const rule3 = rules.find(r => r.id === 'contextualization-reaction-adventure');
