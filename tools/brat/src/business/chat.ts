/**
 * ChatController - Interactive chat with BitBrat platform via WebSocket gateway
 *
 * Sprint 363: Extracted from cli/chat.ts for use by oclif command
 *
 * Supports two modes:
 * - Interactive mode: readline-based terminal chat
 * - One-shot mode: Send single message and exit
 */
...
export interface ChatOptions {
  rootDir?: string;     // Repository root directory (for ContextResolver)
  context?: string;     // Sprint 349+: Execution context name
  env?: string;         // DEPRECATED: Use context instead
  projectId?: string;   // GCP project ID (deprecated, unused)
...
export class ChatController {
  ...
  /**
   * Start chat session (interactive or one-shot)
   */
  ...
  private resolveToken(): string | null {
    ...
    // Try rootDir first, fall back to cwd
    const rootDir = this.options.rootDir || this.findRootDir();
    const configPath = path.join(rootDir, '.bitbrat.json');
  ...
  private async resolveUrl(): Promise<string> {
    ...
    if (this.options.url) {
      ...
      try {
        ...
        const env = this.options.env || 'local';
        if (env === 'local') {
    ...
    this.ws.on('close', (code: number, reason: string) => {
      ...
      if (code !== 1000 && this.reconnectAttempts < this.MAX_RECONNECT_ATTEMPTS) {
        ...
        if (!this.options.message) {
          // Only print disconnect message in interactive mode
