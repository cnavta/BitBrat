...
describe('FleetClient', () => {
  ...
  it('discover() merges fabric + registry so a platform-only Bit (no domain tools) appears', async () => {
    ...
    const registry: RegistryReader = {
      listServers: async () => [
        ...
        { name: 'persistence', profile: 'core', exposure: 'platform-only' }, // platform-only, not surfaced by fabric tools above
      ],
    };
