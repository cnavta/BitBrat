...
/**
 * Generate .env.brat file with required defaults for Docker Compose
 * Sprint 25: Provides defaults for required environment variables
 *
 * @param contextName - Context name
 * @param contextConfig - Context configuration
 * @returns .env.brat file content
 */
export function generateEnvBrat(contextName: string, contextConfig: any): string {
  const persistenceDriver = contextConfig.runtime.persistence.driver;

  return `# .env.brat - Environment Variables for ${contextName}
# Auto-generated by agent-dev provision
# This file provides defaults for required environment variables

# === CORE PLATFORM ===
BITBRAT_VERSION=latest
...
OPENAI_API_KEY=sk-local-dev-stub
LLM_BOT_LLM_API_KEY=sk-local-dev-stub
LLM_BOT_SYSTEM_PROMPT=You are BitBrat, a helpful AI assistant.
QUERY_ANALYZER_LLM_BASE_URL=https://api.openai.com/v1
QUERY_ANALYZER_LLM_API_KEY=sk-local-dev-stub
...
TWITCH_CLIENT_ID=local-twitch-id
TWITCH_CLIENT_SECRET=local-twitch-secret
OAUTH_STATE_SECRET=local-state-secret

# === OPTIONAL INTEGRATIONS (Discord, Slack, Twilio, OBS) ===
# These are optional and will default to empty if not set
# Uncomment and configure as needed for specific testing
...
DEBUG_USERS_DISCORD=
DEBUG_USERS_SLACK=
TWITCH_OAUTH_SCOPES=chat:read chat:edit user:read:email
`;
}
